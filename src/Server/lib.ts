...
export async function logout() {
    ...
    cookies().set("s_role", "", { expires: new Date(0) });
}
...
export const getSession = async () => {
    ...
    try{
        ...
        const roleAuthority = data.authorities.find((authority: { authority: string; }) => authority.authority.startsWith('ROLE_')).authority;

        cookies().set("s_id", data.users.s_id, { expires, httpOnly: true });
        cookies().set("s_uname", data.users.s_uname, { expires, httpOnly: true });
        cookies().set("s_role", roleAuthority, { expires, httpOnly: true });
...
export const getRole = async () => {
    return cookies().get("s_role")?.value
}
